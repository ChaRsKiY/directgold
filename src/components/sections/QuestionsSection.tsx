  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 justify-between gap-12 items-center h-fit max-md:grid-cols-1 max-w-7xl mx-auto">
        ...
        <div id="faq" className="flex justify-evenly h-full flex-1 items-end max-md:items-start">
          <MotionDiv
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="bg-gold h-full min-w-2 rounded-full hidden max-md:block"
          >
          </MotionDiv>
          <div className="flex flex-col gap-8 pl-6 justify-evenly h-full flex-1">
            {questions.map((question, index) => (
              <MotionDiv
                key={index}
                className="flex gap-4 text-right text-gold items-center max-md:text-left w-full justify-end max-md:justify-start"
                initial={{ x: 50, opacity: 0 }}
                whileInView={{ x: 0, opacity: 1 }}
                viewport={{ once: true }}
              ...
              >
                ...
                <MotionDiv
                  className="min-w-3 h-[120%] bg-gold rounded-lg max-md:hidden"
                  initial={{ scaleY: 0 }}
      ...
      <div className="mt-20 flex flex-col items-center gap-4">
        ...
        <MotionDiv
          className="mt-6 text-center text-2xl font-bold text-gold w-2/3 max-md:w-full"
          initial={{ y: 30, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
        ...
        <MotionDiv
          className="text-center text-base text-secondary-text w-2/3 max-md:w-full"
          initial={{ y: 30, opacity: 0 }}
          whileInView={{ y: 0, opacity: 1 }}
          viewport={{ once: true }}
