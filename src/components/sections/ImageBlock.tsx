...
const ImageBlock = ({ side, image, titleKey, descriptionKey }: { side: "left" | "right", image: string, titleKey: string, descriptionKey: string }) => {
  ...
  return (
    <MotionSection
      className={classNames("px-[10%] py-20 flex flex-col gap-12", side === "right" ? "bg-light-bg" : "")}
      initial={{ opacity: 0 }}
    ...
    >
      <MotionH2
        className={classNames("text-6xl max-md:text-4xl font-bold text-gold max-w-7xl mx-auto w-full", side === "right" ? "text-right max-lg:text-left" : "text-left")}
      ...
      <div className={classNames("flex gap-12 justify-between w-full max-lg:flex-col max-lg:gap-6 max-w-7xl mx-auto", side === "right" ? "flex-row-reverse" : "")}>
        <MotionDiv
          className={classNames("text-secondary-text w-[55%] max-lg:w-full", side === "right" ? "text-right max-lg:text-left" : "text-left")}
        ...
        <IoMdArrowDropright size={60} className={classNames("text-gold self-center max-lg:hidden", side === "right" ? "" : "rotate-180")} />
        <MotionDiv
