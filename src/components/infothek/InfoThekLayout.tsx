...
export default function InfoThekLayout({ children }: InfoThekLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-light-bg)] px-[10%] py-20 mt-20">
      ...
      <div className="flex gap-12 max-lg:flex-col">
        ...
        <MotionDiv
          className="w-[300px] max-lg:w-full flex-shrink-0 border-r border-[var(--color-primary-gold)]/20"
          ...
          <nav className="sticky top-24">
            <ul className="space-y-4">
              {navigationItems.map((item, index) => {
                ...
                return (
                  <li key={index} className={classNames(isActive ? "border-r-3 border-[var(--color-primary-gold)] pr-12" : "pr-12")}>
                    <Link
                      href={item.href}
                      className={`block text-lg max-md:text-base leading-relaxed transition-colors duration-300 ${
                        isActive
                          ? "text-[var(--color-primary-gold)] font-semibold"
                          : "text-[var(--color-primary-text)] hover:text-[var(--color-primary-gold)]"
                      }`}
                    >
                      {item.label}
                    </Link>
